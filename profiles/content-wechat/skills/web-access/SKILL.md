...
description: 在当前 Codex/Agents 环境中处理网页搜索、网页读取、社交媒体抓取、登录后浏览、动态页面和真实浏览器交互任务。它是联网策略层：公开网页优先使用内置 `web` 工具，必要时配合 `curl` 或 Jina，涉及登录态、反爬、强动态页面或真实浏览器操作时切换到本地 Chrome CDP Proxy。
metadata:
  author: 一泽Eze
...
当前环境使用 Codex/Agents 的技能目录，而不是 Claude Code 的 `~/.claude/skills`。

常用约定：

```bash
WEB_ACCESS_SKILL_DIR="${WEB_ACCESS_SKILL_DIR:-${CODEX_HOME:-$HOME/.codex}/skills/web-access}"
[ -d "$WEB_ACCESS_SKILL_DIR" ] || WEB_ACCESS_SKILL_DIR="$HOME/.agents/skills/web-access"
WEB_ACCESS_PROXY_PORT="${CDP_PROXY_PORT:-3456}"
WEB_ACCESS_PROXY_BASE="http://127.0.0.1:$WEB_ACCESS_PROXY_PORT"
```

如果另一个更窄的技能已经对来源或工具有硬约束，例如 `openai-docs` 要求官方文档优先，则先遵守那个技能的来源约束；这个 skill 只负责联网路径和浏览器执行策略。
...
- **当前 Codex 环境的基本分工**：公开网页优先走内置 `web` 工具，因为它原生支持搜索、打开结果、站内点击和带来源回答；只有在 `web` 不适合时，再回退到 `curl`、Jina 或本地 CDP。

| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **内置 `web.search_query` / `web.open`** |
| URL 已知，需要读取页面正文、查找具体片段、定位链接 | **内置 `web.open` / `web.find` / `web.click`** |
...
浏览器 CDP 不要求 URL 已知——可从任意入口出发，通过页面内搜索、点击、跳转等方式找到目标内容。内置 `web` 与 `curl` 均不处理登录态，也不等于真实浏览器执行环境。

**Jina**（可选预处理层，可与 `curl` 或内置 `web` 组合使用）：第三方网络服务，可将网页转为 Markdown，大幅节省 token 但可能有信息损耗。调用方式为 `https://r.jina.ai/http://example.com` 或 `https://r.jina.ai/http://...`，限 20 RPM。适合文章、博客、文档、PDF 等以正文为核心的页面；对数据面板、商品页等非文章结构页面可能提取到错误区块。

进入 CDP 浏览器层后，`/eval` 就是你的眼睛和手：
...
```bash
PROXY="http://127.0.0.1:${CDP_PROXY_PORT:-3456}"

# 列出用户已打开的 tab
curl -s "$PROXY/targets"

# 创建新后台 tab（自动等待加载）
curl -s "$PROXY/new?url=https://example.com"

# 页面信息
curl -s "$PROXY/info?target=ID"

# 执行任意 JS：可读写 DOM、提取数据、操控元素、触发状态变更、提交表单、调用内部方法
curl -s -X POST "$PROXY/eval?target=ID" -d 'document.title'

# 捕获页面渲染状态（含视频当前帧）
curl -s "$PROXY/screenshot?target=ID&file=/tmp/shot.png"

# 导航、后退
curl -s "$PROXY/navigate?target=ID&url=URL"
curl -s "$PROXY/back?target=ID"

# 点击（POST body 为 CSS 选择器）— JS el.click()，简单快速，覆盖大多数场景
curl -s -X POST "$PROXY/click?target=ID" -d 'button.submit'

# 真实鼠标点击 — CDP Input.dispatchMouseEvent，算用户手势，能触发文件对话框
curl -s -X POST "$PROXY/clickAt?target=ID" -d 'button.upload'

# 文件上传 — 直接设置 file input 的本地文件路径，绕过文件对话框
curl -s -X POST "$PROXY/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["/path/to/file.png"]}'

# 滚动（触发懒加载）
curl -s "$PROXY/scroll?target=ID&y=3000"
curl -s "$PROXY/scroll?target=ID&direction=bottom"

# 关闭 tab
curl -s "$PROXY/close?target=ID"
```
...
默认保持单 Agent 执行。只有在当前运行环境明确允许，且用户**显式要求**子 Agent、委派或并行 Agent 工作时，才把多个独立目标分治给子 Agent；否则只并行本地工具调用，不主动派生子 Agent。

**好处：**
- **速度**：多子 Agent 并行，总耗时约等于单个子任务时长
- **上下文保护**：抓取内容不进入主 Agent 上下文，主 Agent 只接收摘要，节省 token

**并行 CDP 操作**：如果用户已明确允许子 Agent，每个子 Agent 在当前用户浏览器实例中，自行创建所需的后台 tab（`/new`），自行操作，任务结束自行关闭（`/close`）。所有子 Agent 共享一个 Chrome、一个 Proxy，通过不同 targetId 操作不同 tab，无竞态风险。

**子 Agent Prompt 写法：目标导向，而非步骤指令**
- 只在用户明确允许委派时，才在子 Agent prompt 中写 `必须加载 web-access skill 并遵循指引`。
- 子 Agent 有自主判断能力。主 Agent 的职责是说清楚**要什么**，仅在必要与确信时限定**怎么做**。过度指定步骤会剥夺子 Agent 的判断空间，反而引入主 Agent 的假设错误。**避免 prompt 用词对子 Agent 行为的暗示**：「搜索xx」会把子 Agent 锚定到公开网页搜索，而实际上有些反爬站点需要 CDP 直接访问主站才能有效获取内容。主 Agent 写 prompt 时应描述目标（「获取」「调研」「了解」），避免用暗示具体手段的动词（「搜索」「抓取」「爬取」）。
...
| 需要 CDP 浏览器或长时间运行的任务 | 几次 `web.search_query` / Jina 就能完成的轻量查询 |
