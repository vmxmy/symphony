...
LOG_FILE="${WEB_ACCESS_PROXY_LOG:-/tmp/web-access-cdp-proxy.log}"
...
HEALTH=$(curl -s --connect-timeout 2 "$PROXY_BASE/health" 2>/dev/null)
if echo "$HEALTH" | grep -q '"connected":true'; then
  echo "proxy: ready"
else
  if ! echo "$HEALTH" | grep -q '"ok"'; then
    echo "proxy: starting..."
    nohup env CDP_PROXY_PORT="$PROXY_PORT" node "$SCRIPT_DIR/cdp-proxy.mjs" > "$LOG_FILE" 2>&1 </dev/null &
  fi
  for i in $(seq 1 15); do
    sleep 1
    curl -s "$PROXY_BASE/health" | grep -q '"connected":true' && echo "proxy: ready" && exit 0
    [ $i -eq 3 ] && echo "⚠️  Chrome 可能有授权弹窗，请点击「允许」后等待连接..."
  done
  echo "❌ 连接超时，请检查 Chrome 调试设置；日志：$LOG_FILE"
  exit 1
fi
