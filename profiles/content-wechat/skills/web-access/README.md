...
给 Codex/Agents 装上完整联网能力的 skill。本地副本已按当前环境做过适配。

当前 Codex 环境原本就有内置 `web` 浏览能力，但仍然缺少一层稳定的**联网调度策略 + 真实 Chrome CDP 操作 + 站点经验积累**。这个 skill 补的正是这三部分。
...
```bash
git clone https://github.com/eze-is/web-access ~/.agents/skills/web-access
```
...
node "$WEB_ACCESS_SKILL_DIR/scripts/cdp-proxy.mjs" &

# 页面操作
curl -s "$PROXY/new?url=https://example.com"     # 新建 tab
curl -s -X POST "$PROXY/eval?target=ID" -d 'document.title'  # 执行 JS
curl -s -X POST "$PROXY/click?target=ID" -d 'button.submit'  # JS 点击
curl -s -X POST "$PROXY/clickAt?target=ID" -d '.upload-btn'  # 真实鼠标点击
curl -s -X POST "$PROXY/setFiles?target=ID" \
  -d '{"selector":"input[type=file]","files":["/path/to/file.png"]}'        # 文件上传
curl -s "$PROXY/screenshot?target=ID&file=/tmp/shot.png"     # 截图
curl -s "$PROXY/scroll?target=ID&direction=bottom"           # 滚动
curl -s "$PROXY/close?target=ID"                             # 关闭 tab
```
