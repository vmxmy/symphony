...
- 启动：
  `WEB_ACCESS_SKILL_DIR="${WEB_ACCESS_SKILL_DIR:-${CODEX_HOME:-$HOME/.codex}/skills/web-access}"; [ -d "$WEB_ACCESS_SKILL_DIR" ] || WEB_ACCESS_SKILL_DIR="$HOME/.agents/skills/web-access"; node "$WEB_ACCESS_SKILL_DIR/scripts/cdp-proxy.mjs" &`
...
```bash
PROXY="http://127.0.0.1:${CDP_PROXY_PORT:-3456}"
curl -s "$PROXY/close?target=TARGET_ID"
```
...
```bash
PROXY="http://127.0.0.1:${CDP_PROXY_PORT:-3456}"
curl -s "$PROXY/navigate?target=ID&url=https://example.com"
```
...
```bash
PROXY="http://127.0.0.1:${CDP_PROXY_PORT:-3456}"
curl -s -X POST "$PROXY/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["/path/to/file1.png","/path/to/file2.png"]}'
```
