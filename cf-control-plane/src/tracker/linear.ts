// LinearGraphqlClient — Worker-runnable port of ts-engine/src/linear.ts.
//
// Phase 3 deliverable: provides fetchActiveIssues / fetchTerminalIssues /
// fetchIssuesByIds against the Linear GraphQL API using only the
// Worker-compatible `fetch` API. Query strings and response normalization
// are byte-equivalent to the ts-engine client so the reconciliation
// harness produces identical Decision[] for identical tracker inputs.

import type { Issue, BlockedRef } from "../types.js";
import type {
  TrackerAdapter,
...
export class LinearGraphqlClient implements TrackerAdapter, RawTrackerToolAdapter {
  constructor(private cfg: LinearTrackerConfig) {
    if (!cfg.apiKey) {
      throw new Error("linear_no_api_key: LINEAR_API_KEY secret is not configured");
    }
  }
